...
type AutoSellViewFilter = "all" | "selected" | "needs_price" | "priced";
...
export default function Page() {
  ...
  const [showAutoSellShippingOptions, setShowAutoSellShippingOptions] = useState(false);
  const [autoSellSearch, setAutoSellSearch] = useState("");
  const [autoSellViewFilter, setAutoSellViewFilter] = useState<AutoSellViewFilter>("all");
  ...
  async function openAutoSellModal() {
    ...
    setShowAutoSellShippingOptions(false);
    setAutoSellSearch("");
    setAutoSellViewFilter("all");
  ...
  function selectAutoSellPricedOnly() {
    setAutoSellDrafts((prev) =>
      prev.map((draft) => ({
        ...draft,
    ...
    setAutoSellViewFilter("priced");
  }

  function clearVisibleAutoSellPrices() {
    const visibleIds = new Set(visibleAutoSellDrafts.map((draft) => draft.cardId));
  ...
  const visibleAutoSellDrafts = useMemo(() => {
    ...
    return autoSellDrafts.filter((draft) => {
      ...
      const matchesView =
        autoSellViewFilter === "all"
          ? true
          : autoSellViewFilter === "selected"
            ? draft.selected
            : autoSellViewFilter === "needs_price"
              ? draft.price.trim() === ""
              : draft.price.trim() !== "";

      return matchesSearch && matchesView;
    });
  }, [autoSellDrafts, autoSellSearch, autoSellViewFilter, cards]);
  ...
  return (
    <main className="collectionPage">
      <style jsx>{`
        ...
        .autoSellMetaBadges span.ready {
          background: #ecfdf5;
          border-color: #bbf7d0;
          color: #065f46;
        }

        .autoSellMetaBadges span.needs {
          background: #fff7ed;
          border-color: #fed7aa;
          color: #9a3412;
        }
        ...
        @media (max-width: 920px) {
          ...
          .autoSellQuickActions .autoSellSmallButton {
            width: 100%;
            white-space: normal;
          }
          ...
          <div className="eliteModalOverlay autoSellOverlay">
            <div className="eliteModal autoSellModal">
              ...
              <div className="autoSellModalHeader">
                ...
                <div>
                  <h2 className="eliteModalTitle autoSellModalTitle">Manage extras to list</h2>
                  <div className="eliteModalText autoSellModalText">
                    Search, filter, select, price, and post your extras without losing track of what is being listed.
                  </div>
                </div>
              </div>
              ...
              <div className="autoSellSummaryGrid">
                <button
                  type="button"
                  className={`autoSellSummaryCard ${autoSellViewFilter === "all" ? "active" : ""}`}
                  onClick={() => setAutoSellViewFilter("all")}
                >
                ...
                <button
                  type="button"
                  className={`autoSellSummaryCard ${autoSellViewFilter === "selected" ? "active" : ""}`}
                  onClick={() => setAutoSellViewFilter("selected")}
                >
                  <span>Selected</span>
                ...
                <button
                  type="button"
                  className={`autoSellSummaryCard ${autoSellViewFilter === "priced" ? "active" : ""}`}
                  onClick={() => setAutoSellViewFilter("priced")}
                >
                  <span>Priced</span>
                ...
                <button
                  type="button"
                  className={`autoSellSummaryCard ${autoSellViewFilter === "needs_price" ? "active" : ""}`}
                  onClick={() => setAutoSellViewFilter("needs_price")}
                >
              ...
              <div className="autoSellManager">
                ...
                <div className="autoSellQuickActions">
                  <button
                    type="button"
                  ...
                  <button
                    type="button"
                    className="autoSellSmallButton"
                    onClick={selectAutoSellPricedOnly}
                  >
                    Select priced only
                  </button>
                  ...
                  <button
                    type="button"
                    className="autoSellSmallButton danger"
                    onClick={clearVisibleAutoSellPrices}
                  >
              ...
              <div className="autoSellToolbar">
                <div className="autoSellHint">
                  Showing {visibleAutoSellDrafts.length} of {autoSellDrafts.length}. Prices auto-fill only when that Doorable already has sold listings with an average sale price.
                </div>
              ...
              <div className="autoSellShippingPanel">
                <button
                  type="button"
                  className="autoSellShippingToggle"
                  onClick={() => setShowAutoSellShippingOptions((prev) => !prev)}
                >
                  <span>
                    Shipping: {autoSellShippingAvailable ? "Available" : "Not offered"}
                    {autoSellShippingPrice.trim() ? ` • $${autoSellShippingPrice.trim()}` : ""}
                    {autoSellLocalPickupAvailable ? " • Local pickup" : ""}
                  </span>
                  <strong>{showAutoSellShippingOptions ? "Hide" : "Edit"}</strong>
                </button>

                {showAutoSellShippingOptions && (
                  <>
                    <div className="autoSellShippingText">
                      Applies to every listing in this batch. Keep it simple — buyers can still message you.
                    </div>
              ...
              <div className="autoSellListHeader">
                <div>
                  <strong>Items being listed</strong>
                  <span>Scroll this section to review each extra. Filter chips above keep this manageable.</span>
                </div>
              </div>
              ...
              <div className="autoSellModalList">
                {visibleAutoSellDrafts.length === 0 ? (
                  <div className="autoSellEmptyState">
                    No extras match this view. Try clearing the search or changing the filter.
                  </div>
                ) : (
                    ...
                    return (
                      <div key={draft.cardId} className={`autoSellItem ${draft.selected ? "selected" : ""}`}>
                        <label className="autoSellCheck">
                          ...
                          />
                          <span>{draft.selected ? "Listed" : "Skip"}</span>
                        </label>
                        ...
                        <div className="autoSellFields">
                          <div className="autoSellTitlePriceRow">
                            ...
                            <input
                              className={`autoSellInput priceInput ${hasPrice ? "hasPrice" : ""}`}
                              value={draft.price}
                              onChange={(e) =>
                                updateAutoSellDraft(draft.cardId, {
                          ...
                          <div className="autoSellMetaBadges">
                            ...
                            {draft.selected ? <span className="ready">Selected</span> : <span>Skipped</span>}
                          </div>
                        </div>
                      </div>
                    );
