                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div className="listControls">
                      <div className="listQtyControls">
                        <button type="button" onClick={() => void saveCard(item, item.qty - 1, item.note)} disabled={savingId === item.id} className="listQtyButton" aria-label={`Decrease quantity for ${item.name}`}>−</button>
                        <input
                          type="number"
                          min="0"
                          value={item.qty}
                        ...
                        />
                        <button type="button" onClick={() => void saveCard(item, item.qty + 1, item.note)} disabled={savingId === item.id || !canAdd} className="listQtyButton" aria-label={`Increase quantity for ${item.name}`} style={{ opacity: !canAdd ? 0.45 : 1, cursor: !canAdd ? "not-allowed" : "pointer" }}>+</button>
                      </div>
            ...
            return (
              <div key={item.id} className="card" style={{ background: `${subtleOverlay}, ${rarity.bg}`, color: rarity.text, borderColor: rarity.border, boxShadow: `0 12px 28px rgba(0,0,0,0.14), 0 0 18px ${rarity.glow}` }}>
                ...
                <div className="qtyRow">
                  <button type="button" onClick={() => void saveCard(item, item.qty - 1, item.note)} disabled={savingId === item.id} className="qtyButton" aria-label={`Decrease quantity for ${item.name}`}>−</button>
                  <input
                    type="number"
                    min="0"
                    value={item.qty}
                    ...
                    aria-label={`Quantity owned for ${item.name}`}
                  />
                  <button type="button" onClick={() => void saveCard(item, item.qty + 1, item.note)} disabled={savingId === item.id || !canAdd} className="qtyButton" aria-label={`Increase quantity for ${item.name}`} style={{ opacity: !canAdd ? 0.45 : 1, cursor: !canAdd ? "not-allowed" : "pointer" }}>+</button>
                </div>
